<template>
	<div v-if="furnitureList.length!=0">
		<a href="#one"></a>

		<swipe class="my-swipe">
			<swipe-item v-for="data of furnitureList[0].moduleContent.banners" :key="data.id">
			  	<img class="myswipe" :src="data.bannerImgSrc" alt="">
			</swipe-item>
		</swipe>

		<div class="th1">
			<h2>{{furnitureList[1].moduleName}}</h2>
			<p>{{furnitureList[1].moduleDescription}}</p>
			<div class="small" @touchstart="handleTouch">
				<div v-for="data in furnitureList[1].moduleContent.products">
		...
		<div class="th2">
			<h2>{{furnitureList[2].moduleName}}</h2>
			<p>{{furnitureList[2].moduleDescription}}</p>
			<img :src="furnitureList[2].moduleContent.banners[0].bannerImgSrc" alt="">
			<div v-for="info in furnitureList[2].moduleContent.products" class="ads">
		...
		<div class="th3">
			<h2>{{furnitureList[3].moduleName}}</h2>
			<div v-for="data in furnitureList[3].moduleContent.banners">
				<img :src="data.bannerImgSrc" alt="">
			</div>
		</div>

		<div class="th4" v-for="data,index in furnitureList" v-if="index>=4">
	...
	export default {
		...
		methods:{
			handleTouch(e){
				...
				var count=this.furnitureList[1].moduleContent.products.length;
		...
		mounted(){
			if(this.$store.state.furnitureList.length===0){
				this.$store.dispatch("furnitureAction");
			}
